	...
	PSTR szCmdLine, int iCmdShow)
{
	...
	iFile = CreateFile(".\\key.txt", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	DWORD iOutput;
	...
	SetFilePointer(iFile, 0, NULL, FILE_END);
	WriteFile(iFile, szTop, lstrlen(szTop), &iOutput, NULL);
	WriteFile(iFile, "\r\n", 2, &iOutput, NULL);
	WriteFile(iFile, szUnd, lstrlen(szUnd), &iOutput, NULL);
	WriteFile(iFile, "\r\n", 2, &iOutput, NULL);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DISPLAYCHANGE:
		...
		//[2005-04-28] Chj: The memory is not freed before the program's quit. 
		//Where should I free it(in response to which WM_ message)?
		cLines = 0;
		// fall through
	case WM_SIZE:
		if (message == WM_SIZE)
	...
	case WM_SYSDEADCHAR:
		...
		GetKeyNameText(pmsg[0].lParam, szKeyName,
			sizeof(szKeyName) / sizeof(TCHAR));
		iLen = wsprintf(szBuffer, szFormat[iType],
		...
		WriteFile(iFile, szBuffer, iLen, &iOutput, NULL);
		WriteFile(iFile, "\r\n", 2, &iOutput, NULL);
